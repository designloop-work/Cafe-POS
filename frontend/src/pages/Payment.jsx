  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex overflow-hidden">
                ...
                <motion.div key={o.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  className={`cursor-pointer rounded-2xl border p-4 transition-all duration-200
                    ${selected?.id === o.id
                      ...
                      : 'border-surface-border bg-surface-1 hover:border-gray-600 hover:bg-surface-2'}`}
                >
        ...
        {/* Payment panel */}
        <div className="w-80 border-l border-surface-border p-5 flex flex-col gap-4 bg-surface-1/40">
                      ...
                      <button key={id} onClick={() => setMethod(id)}
                        className={`flex flex-col items-center gap-2 p-3.5 rounded-xl border transition-all duration-200
                          ${method === id
                            ...
                            : 'border-surface-border bg-surface-2 text-gray-400 hover:border-gray-500'}`}>
                        <Icon size={20} />
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                      ...
                      <div className="w-24 h-24 bg-white rounded-lg mx-auto mb-2 flex items-center justify-center">
                        <div className="grid grid-cols-5 gap-0.5">
                          {[...Array(25)].map((_, i) => (
                            <div key={i} className={`w-3.5 h-3.5 rounded-sm ${Math.random() > 0.5 ? 'bg-gray-900' : 'bg-white'}`} />
                          ))}
                        </div>
                      </div>
