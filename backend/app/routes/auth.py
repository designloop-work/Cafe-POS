...
async def require_admin(user=Depends(get_current_user)):
    if user.get("role") not in (UserRole.admin, "admin"):
        raise HTTPException(status_code=403, detail="Admin access required")
    return user

@router.post("/register", response_model=UserOut)
async def register(data: UserCreate):
    """Public registration — staff by default, admin if correct admin_secret provided."""
    db = get_database()
    ...
    if data.admin_secret:
        if not settings.ADMIN_REGISTER_SECRET:
            raise HTTPException(status_code=400, detail="Admin registration is not enabled")
        if data.admin_secret != settings.ADMIN_REGISTER_SECRET:
